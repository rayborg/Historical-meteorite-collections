...
Each of the 273 entries identifies one individual Table A specimen. Specimen IDs are exact, unique canonical Victoria Land identifiers; every mass is positive and counted once. The records contain locality name/code/nullable coordinate and nullable mineral/weathering facts, but no name, MetBull mapping, lineage, holding, custody, or ownership claim. Their 273 masses total 969,562.2 g.
...
The standard fact order is optional differing current Meteoritical Bulletin name, class, specimen form for specimen kinds, source locality, event, lineage and specimen weight for specimen kinds. Missing values use `Not recorded`. Collection and regional observations omit form, lineage, and specimen weight. Catalog-specific holdings, provenance, coordinates, mineral chemistry, sections, totals, amendments, representation facts, occurrence counts, and source prose are not copied into standard card fields, but remain in the validated public record and searchable index.

`Specimen form` is determined only by the reviewed presenter kind. `Source locality` is a source-scoped catalog fact, not an individual find-location field. Coordinates, general locality text, MetBull mappings, and descriptive prose cannot populate an individual find location or change specimen form. The current public data and presenter make no claim that an exact individual find location is available.
