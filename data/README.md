...
The root contains exactly `metadata` and `records`. `metadata.schemaVersion` is 15 and each catalog descriptor declares one of eleven `recordModel` values.
...
The other specimen additions retained in schema 15 are closed evidence fields. One direct Chicora record carries `weightEvidence: { type: "qualitative", statement: "Less than a gram" }`. Lakewood carries a 5,265.8 g specimen weight plus separate `associatedMaterial` for a 1,813.3 g fragment box whose count was not reported. Three reviewed plural terrestrial-material records carry `specimenDisposition.type: "not-individual"` and render as source observations rather than specimens. The validator binds all of these values to their accepted record IDs.

### Appendix Specimen

Used only by `antarctic-1980`:

```text
id, catalogId, entryOrder, specimenId, name, weight: { grams },
classification, olivineFa, pyroxeneFs, weathering,
locality: { code, name, areaReferenceCoordinate }, catalogPage,
confidence, metbull
```

The descriptor is bounded to Appendix 1 of *Catalog of Antarctic Meteorites, 1977-1978* on printed pages 47-48; Appendix 2 and all other source matter are excluded. Its 85 independent source-order observations comprise 54 rows on page 47 and 31 on page 48. Each retains a unique `ALHA77` specimen ID as both `specimenId` and source `name`, one positive source mass, source classification, the exact Allan Hills locality object (`ALH`, `Allan Hills`, `77°S, 159°E`), nullable source olivine Fa, pyroxene Fs, and weathering, and an accepted `official-abbreviation` MetBull mapping. The masses total 89,891.1 g with no null mass; null counts are 9 olivine Fa, 7 pyroxene Fs, and 6 weathering values. Null means the source value is unavailable and never means zero or permission to infer a value.

All 85 IDs independently recur in the later `victoria-land-1982` Table A. They remain separate catalog observations. Their exact `same-inventory` links establish identifier continuity only, not an unchanged physical specimen or mass, custody, ownership, transfer, or a merge. Antarctic Appendix masses are excluded from cross-catalog comparison generation. Current MetBull context is attached only after the reviewed code binding and cannot replace any Appendix 1 source fact or create continuity.
...
Used only by `victoria-land-1982`:

```text
id, catalogId, entryOrder, specimenId, name, weight: { grams }, classification,
olivineFa, pyroxeneFs, weathering, locality: { code, name, areaReferenceCoordinate },
catalogPage, sourceEvidence: { primary, tableA, tableB, conflicts }, confidence, metbull
```

Each of the 273 entries identifies one individual Table A specimen. Specimen IDs and source names are equal exact full identifiers; every record has one accepted `official-abbreviation` MetBull mapping. Every primary Table A mass is positive and counted once. The descriptor covers printed pages 85-94, with Table A records citing pages 85-88 and all normalized Table A, Table B, and Table C references constrained to the descriptor. The closed normalized `sourceEvidence` object identifies Table A as primary, repeats its normalized page and facts, retains an optional normalized Table B cross-view, and lists only recomputable `classification`, `mass`, or `weathering` conflicts. Top-level facts must exactly equal Table A. Table B covers 270 records, with classification on 268, weathering on 249, fracturing on 250, and two `Unclassified` classification contexts. The release contains 40 mass, 2 classification, and 8 weathering conflicts. Their 273 primary masses total 969,562.2 g.

### Dealer Offer Fact

Used only by `foote-1909`:

```text
id, catalogId, typeNumber, name, description, catalogPage, confidence
```

The six records are source dealer-type observations, not specimens or holdings. They contain no price, mass, MetBull mapping, lineage, custody, ownership, transfer, or merge claim.

### Caption Observation Fact

Used only by `haag-2003`:

```text
id, catalogId, entryOrder, captionTitle, name, classification,
reportedMass: { valueText, semantics } | null,
reportedDimensions: { valueText, semantics } | null,
photoPanelCount, sourceRelation, catalogPages, printedPageLabel, confidence
```

The 250 source-order records are caption observations across 134 cited pages, not specimens, holdings, inventory records, or current-custody claims. Reported mass and dimensions are retained only as `caption-context-only` text. A nonnull `sourceRelation` can express only a repeated view of an earlier public Haag caption record. The model prohibits MetBull mappings and contributes no specimen cards, projections, lineage endpoints, or comparison candidates.
...
The runtime derives 25,244 closed display DTOs without changing parent result counts: 5,824 `direct-specimen`, 8,410 `projected-atomic-specimen`, 6,867 `collection-observation`, 3,447 `collection-representation-observation`, 250 `caption-observation`, 84 `regional-observation`, 353 `regional-event-observation`, 6 `dealer-observation`, and 3 `source-observation` cards. Every DTO contains exactly `kind`, `identifier`, `semanticLabel`, `sourceName`, `headingName`, `headingLabel`, `headingUrl`, `description`, `facts`, `catalogNotes`, `sourceCitation`, `sourceLabel`, `catalogId`, `catalogPages`, `lineage`, and `comparison`; every fact and catalog-note entry contains exactly `label` and `value`.

Every one of the 25,244 display identifiers starts with the established concise catalog label. Existing specimen and projection identifiers remain unchanged. Haag's 250 observations append the explicit source-scoped label `Caption observation <entryOrder>`; this does not create a specimen or inventory identifier.

Mapped specimen DTOs use the Official current MetBull name as `headingName` with the **Current MetBull meteorite name** label and canonical code URL. When the exact source name differs, it becomes the first primary fact and displays as **Source name**. Current event-context facts then follow in classification, place, fall/find, and year order. Source-specific specimen form, individual find location, lineage, comparisons, and specimen weight remain primary facts when applicable. Differing catalog classification, source locality, and event enter `catalogNotes`; the renderer exposes them through initially collapsed **Show catalog notes**, and search continues to index them. The specimen renderer alone substitutes compact visible heading, fact, note, and source labels while retaining every complete DTO label in visually hidden assistive text; unknown labels pass through unchanged. Observation rendering does not use this compactor. Equivalent source context is not duplicated.

Unmapped specimens, observations, and all cards under sidecar failure remain source-only. Their headings retain kind-correct meteorite, source, meteorite-or-locality, or catalog labels, and source-only specimen facts retain catalog classification, source locality, and event. Complete field-aware unavailability values are omitted, while compound values retaining evidence remain unchanged. Source citations omit unavailable page wording. Collection, regional, dealer, and not-individual source observations omit specimen-only fields. Expanded lineage details omit stable relationship/group IDs and unavailable endpoint/review rows without changing lineage DTOs or routing. Foote dealer descriptions and Lakewood's controlled main-mass/aggregate explanation are the only populated public card descriptions in this release. Other catalog-specific holdings, provenance, coordinates, mineral chemistry, sections, totals, amendments, representation facts, and occurrence counts remain searchable rather than becoming standard card fields.

`Specimen form` is determined only by the reviewed presenter kind. The 8,768 projected atomic, Appendix, or Table A cards display `Individual specimen`; 5,466 generic specimen cards omit the redundant `Specimen form: Specimen` row. `Source locality` is a source-scoped catalog fact, not an individual find-location field; on mapped cards it remains in collapsed catalog notes when it differs from current Official place. Coordinates, general locality text, MetBull mappings, and descriptive prose cannot populate an individual find location or change specimen form. The 111 typed specimen values display exactly; the other 14,123 specimen cards omit that unavailable row.

## Specimen Card Projections

`specimen-card-projections.json` is a schema-6, display-only positive allowlist with `catalogSchemaVersion: 15`, source count 20,241, and exact catalog hash `3c23d1c2653bc893819a605c7c0e5e6db7b63a17cd3ab66ab104c2772391dbe7`. Each card references an immutable parent observation and exact public source evidence through the closed clause, component, or Bonn projection contract. Optional `sourceCatalogNumber` evidence contains exactly an opaque nonempty source value, same-holding public text path, and valid UTF-16 half-open bounds. The manifest contains 3,407 reviewed parent projections, 8,410 atomic cards, 2,877 derived source-context audit partitions, and 5,058 reviewed numbers across Farrington 1903 and 1916, Reeds 1937, Prior 1923, Tassin 1902, and one Merrill 1916 card. Nininger's 241 projected typed designations are not duplicated. Parent-row values, ambiguous/multiple values, external references, private-only values, and source absences remain unannotated. These six catalogs currently have blocked or undetermined folio policies, so visual source comparison requires the cited source edition rather than an in-app facsimile. Of all cards, 8,375 have ordinary mass paths, 2 Kuleschowka cards use repeated mass, and 35 have no normalized display mass, including 10 qualitative cards. Bonn contributes 348 reviewed cards across 345 parents, including seven qualitative `Spl.` cards; its collection totals remain aggregate context and never become cards or mass paths. Fletcher, Story, Prior represented weights, regional-event material, and Haag captions remain context and do not enter specimen mass filtering or comparisons.
...
`metbull-current-context.json` is schema 1 and has SHA-256 `18ee18a7396c06d928a30580efc1dfa2c585c0446ae85796b42ce85e44b0ac0e`. It provides current meteorite-event context for the 14,234 specimen cards only: 11,859 mapped cards from 7,736 mapped parents use 2,548 Official codes, while 2,375 cards remain unmapped. The direct partition is 5,653 mapped plus 171 unmapped; the projected partition is 6,206 mapped plus 2,204 unmapped.
...
Across different collection namespaces, comparison candidates retain the reviewed event-identity-plus-mass method. Endpoints must share a reviewed MetBull code or, only when both are unresolved, the same normalized source name. A MetBull code identifies the meteorite event, not an individual physical specimen. Source-name normalization applies NFKD, removes diacritics, lowercases, replaces non-alphanumeric runs with spaces, and trims. Exact masses have zero difference. Non-exact near masses require a minimum endpoint mass of 10 g, relative difference at most 0.0025, and absolute difference at most 2 g. Mass is comparison evidence only: exact or near mass does not prove physical identity or lineage.

`specimen-comparison-reviews.json` is the formal public review source for comparison candidates, and `specimen-comparison-reviews.schema.json` is its machine-readable contract. Each review is keyed by its stable comparison candidate ID; generated candidates separately retain their schema-3 candidate ID for migration traceability. Outcomes are limited to `retain-as-possible` and `not-supported`; unlisted candidates remain unreviewed. Same-inventory relationships do not use comparison reviews.

The generator flattens every positive numeric mass without multiplying by holding count; zero values remain catalog facts but are not comparison mass endpoints. Scalar specimen masses use `weight.grams`; catalog-item masses use `holdings[i].weight.grams`; catalog-number and collection-entry masses use `holdings[i].weights[j].grams`. Inventory observations additionally retain the exact `designationPath`, including observations whose source mass is null. Every generated observation and runtime card link uses the project-relative `record id <recordId>` query, so duplicate source labels or designations still resolve to exactly one public record.

The current public input produces 17,160 positive flattened mass observations and 3,985 inventory observations. The artifact contains 279 same-inventory relationships, including exactly 85 Antarctic-to-Victoria links. Antarctic Appendix masses do not enter comparison generation. It also embeds the exact 21 tentative Table C groups, covering 87 unique full IDs and 89 member occurrences, as n-ary `sourceAttestedGroups` bound to source-claims content SHA-256 `141ed60b9560596ac8ab392babfc4af6e1d22921bacbf979d3b975e0fc2f20c2`; groups are never pairwise-expanded. Separately, the unchanged 1,541 cross-catalog comparison groups contain 2,245 candidates: 1,323 groups are singleton and 218 are ambiguous; 2,014 candidates use exact mass and 231 use near mass; 906 are reviewed `retain-as-possible` and 1,339 remain unreviewed. The 10 reviewed Bonn candidates remain comparisons only. Groups preserve competing alternatives without pairwise false precision; Holbrook 98 is one ambiguity group. No comparison asserts lineage, physical identity, custody, ownership, transfer, or merge.

Card routing is source-complete rather than summary-only. The lineage-only view contains exactly 303 cards and 358 claims: 279 `known` same-inventory claims and 79 `tentative` source-attested group occurrences. Cross-catalog comparisons never enter lineage-only. Separate comparison routing contains 1,717 cards and 1,889 top-level comparison-group occurrences. Same-inventory claims retain closed endpoints and their normalized collection-series basis; source-attested claims retain the complete n-ary group and source citation; comparison groups retain candidates, evidence, cautions, and public review details. Controlled warnings state that shared event identity and reported mass do not establish a common physical specimen, lineage, custody, ownership, transfer, or merge.

## Source Claims

`source-claims.json` is a closed schema-1 artifact containing exactly 21 tentative n-ary Appendix Table C groups. Its 89 member occurrences cover 87 unique exact full Victoria specimen IDs. The artifact retains source group order, classification, reference, and printed page 94 without suffix expansion, pairwise conversion, raw text, private page IDs, source files, or assets. `source-claims.schema.json` provides the JSON Schema contract; `validate-source-claims.mjs` additionally enforces exact accepted content and catalog binding.

Relationship, group, candidate, and observation IDs are deterministic UUIDv5 values under the fixed namespace `65b19e0b-1f86-5ca5-a65b-81c38ec53040`. They use public record IDs and the applicable exact designation or mass paths, series IDs, normalized inventory IDs, identity partitions, catalog pairs, and reported mass pairs rather than review state, score, or output order. The browser validator independently reconstructs the complete inventory relationships and comparison groups before displaying this optional enhancement. Regenerate with `node scripts/build-specimen-lineages.mjs`; use `--check` to detect byte drift. Validate independently with `node scripts/validate-specimen-lineages.mjs`.
...
| `antarctic-1980` | blocked | undetermined | 0 |
...
| `haag-2003` | blocked | undetermined | 0 |
