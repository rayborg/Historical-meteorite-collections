...
`catalog.json` is a schema-6 facts-only dataset containing 5,507 source observations from 18 historical meteorite catalogs. `folios.json` is a separate schema-2, deny-by-default display manifest with 49 reviewed page entries.
...
The current public input produces 3,916 flattened mass observations and 3,627 inventory observations. It publishes 241 relationships: 195 same-inventory relationships and 46 possible cross-source matches. The same-inventory total comprises 2 Huss and 193 Nininger relationships; one duplicated key is identity-resolved and none are omitted as ambiguous. The 46 possible matches include 19 exact-mass and 27 near-mass candidates, all currently unreviewed and based on reviewed MetBull identity.
