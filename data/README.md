...
`catalog.json` is a schema-6 facts-only dataset containing 9,050 source observations from 20 historical meteorite catalogs. `folios.json` is a separate schema-2, deny-by-default display manifest with 49 reviewed page entries.
...
Chladni 1825 pages 200-207 are introductory folios. `haidinger-1859` page 21 introduces the holdings list, whose entries begin on page 22. `buchner-1863` covers the Vienna register on Roman pages XIII-XVII, from Alais through Hemalga. `tassin-1902` metadata includes plate page 671 and introductory page 673; its 340 entries cite pages 675-698. `schreiter-1912` metadata spans pages 58-75; its 162 entries cite pages 66-73. `mason-1964` metadata spans pages 1-40; its 1,374 entries cite 33 of those pages. `asu-2024-09` contributes 2,169 facts-only records citing all 53 dataset pages, with 2,166 unique designations and only `91`, `157`, and `607` duplicated. `nininger-1933` includes printed pages 1-20; pages 12-20 are narrative-only, and the printed catalog numbering skips item 139.
...
Used by Lucas, both Chladni catalogs, Haidinger, Buchner, Nordenskiöld, Ball, Clarke, Washington, Tassin, Högbom, Farrington, and Schreiter:
...
The current public input produces 3,916 flattened mass observations and 3,627 inventory observations. It publishes 241 relationships: 195 same-inventory relationships and 46 possible cross-source matches. The same-inventory total comprises 2 Huss and 193 Nininger relationships; one duplicated key is identity-resolved and none are omitted as ambiguous. The 46 possible matches include 19 exact-mass and 27 near-mass candidates, all currently unreviewed and based on reviewed MetBull identity. ASU September 2024 adds no lineage observations because it has no reviewed MetBull mappings and is not a registered edition series; the generated relationship set is unchanged.
