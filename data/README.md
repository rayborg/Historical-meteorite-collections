...
`catalog.json` is a schema-6 facts-only dataset containing 13,542 source observations from 33 historical meteorite catalogs. `folios.json` is a separate schema-2, deny-by-default display manifest with 49 reviewed page entries.
...
Used by Lucas, both Chladni catalogs, Haidinger, Buchner, Nordenskiöld, Ball, Clarke, Washington, Tassin, Högbom, Ward, both Farrington catalogs, Schreiter, Foote, Anderson, Merrill, Kantor, Prior, Palache, Nininger, Reeds, Astapovich, Barnes, and Kanagawa:
...
`specimen-card-projections.json` is a schema-2, display-only positive allowlist. Each card references an immutable parent observation, an exact public `holdingPath`, and a reviewed UTF-16 clause span in that holding's public description or designation. Its nullable `massPath` binds at most one normalized mass. The manifest never creates a new observation ID, changes `catalog.json`, or copies source prose. It contains 1,790 reviewed parent projections, 6,403 atomic specimen cards, and 1,510 derived source-context cards; 134 atomic cards preserve source-supported uncertain-size clauses without claiming normalized grams.

Projection order follows canonical parent, holding, and clause order. Exact non-null mass paths also scope displayed lineage. Ranges, totals, grouped counts, casts, aggregates, and unsupported material remain in a separately labelled source-context card. Reeds 1937 entry 366 is locked to ten ordered atomic cards without context. Prior 1923 entry 630 is locked to seventeen direct specimen clauses and one context card: twelve clauses bind normalized masses, five preserve uncertain source-reported sizes without normalized grams, and the four normalized range endpoints and totals remain context. Parent observation counts, statistics, citations, folio authorization, and lineage records remain unchanged.
...
The current public input produces 13,706 flattened mass observations and 3,627 inventory observations. It publishes 1,482 relationships: 195 same-inventory relationships and 1,287 possible cross-source matches. The same-inventory total comprises 2 Huss and 193 Nininger relationships; one duplicated key is identity-resolved and none are omitted as ambiguous. The possible matches include 1,065 exact-mass and 222 near-mass candidates, all currently unreviewed and based on reviewed MetBull identity. Foote participates in 16 candidates, 15 exact-mass and 1 near-mass, while adding no same-inventory claim. The completed Anderson, Kantor, and Astapovich reviews add no relationship, and all 1,482 baseline relationship objects remain unchanged.
