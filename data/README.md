...
Every mapped or unmapped specimen DTO uses `Name` as `headingLabel` and only `Class`, `Place`, `Fall / find`, `Year / date`, `Form`, `Find location`, and `Weight` as fact labels. Mapped current values take precedence; a recorded source value is used only when current context is absent or unavailable. Unavailable values are omitted without inference. Differing source name/class/place/date, source description, Victoria conflicts, locality/grid/chemistry/weathering/source-section details, lineage, and comparisons remain in `catalogNotes` or their existing nested DTOs and render inside initially collapsed **Show specimen notes**. Observation rendering and **Show catalog notes** remain unchanged. Equivalent source context is not duplicated.

Unmapped specimens and all specimen cards under sidecar failure remain source-only under the neutral specimen contract. Complete field-aware unavailability values are omitted, while compound values retaining evidence remain unchanged. Source citations omit unavailable page wording. Collection, regional, dealer, and not-individual source observations omit specimen-only fields. Expanded lineage and comparison details remain available inside specimen notes while omitting stable relationship/group IDs and unavailable endpoint/review rows. Foote dealer descriptions remain observation descriptions; specimen descriptions and catalog-specific details do not become primary fields.

`Form` is determined only by reviewed presenter kind. The 8,768 projected atomic, Appendix, or Table A cards display `Individual specimen`; 5,466 generic direct cards display `Specimen`. General source locality may supply `Place` but cannot populate `Find location`. Coordinates, MetBull mappings, and descriptive prose cannot populate a find location or change form. The 111 typed specimen locations display exactly; the other 14,123 specimen cards omit that unavailable row.
