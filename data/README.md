...
`catalog.json` is a schema-6 facts-only dataset containing 1,984 source observations from eight historical meteorite catalogs. `folios.json` is a separate schema-2, deny-by-default display manifest with 41 reviewed page entries.
...
Metadata source-page coverage is not a count of pages cited by records. For `nininger-1933`, printed pages 8-9 and catalog items 106-141 are missing from the available source set. Pages 12-20 are narrative-only. Its 136 represented items contain 310 reviewed holdings: 298 designated, 12 anonymous, 304 gram-bearing, 4 casts, and 2 count-only. `nininger-1950` currently covers printed pages 26-31 with 72 specimen observations through Bear Creek; the following Beardsley heading has no page-31 specimen row and therefore no observation on that page. Both Nininger catalogs are partial digital editions.
