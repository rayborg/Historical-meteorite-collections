...
Every displayed source identifier is scoped with the established concise catalog label, such as `Nininger (1933) · 9b`, without changing the stored source value, search index, or sort semantics. Every specimen uses this known-fact order: current Meteoritical Bulletin name, class, specimen form, source locality, individual find location, event, lineage, and specimen weight. Missing values and missing source identifiers are omitted rather than rendered as `Unknown`. Collection, regional, dealer, and not-individual source observations omit specimen-only fields. Foote dealer descriptions and Lakewood's controlled main-mass/aggregate explanation are the only populated public card descriptions in this release. Other catalog-specific holdings, provenance, coordinates, mineral chemistry, sections, totals, amendments, representation facts, and occurrence counts remain searchable rather than becoming standard card fields.
