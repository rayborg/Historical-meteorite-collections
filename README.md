...
This repository is a dependency-free, facts-only index of 20,241 source observations from 56 historical meteorite catalogs. The coordinated catalog uses public metadata schema 15 and supports 11 source-specific record models: `appendix-specimen`, `caption-observation-fact`, `catalog-item`, `catalog-number`, `collection-entry`, `collection-representation-fact`, `dealer-offer-fact`, `regional-census-fact`, `regional-event-fact`, `specimen`, `table-a-specimen`. Reviewed projection metadata expands 3,407 parent observations into 8,410 atomic specimen cards, producing 25,244 searchable display descriptors: 14,234 specimen cards and 11,010 observations.
...
Ursula B. Marvin and Brian Mason compiled *Catalog of Antarctic Meteorites, 1977-1978* (1980). This release is bounded to its Appendix 1 specimen table on printed pages 47-48; Appendix 2 and all other source matter are outside the `antarctic-1980` public record scope.
...
The browser loads factual records from `./data/catalog.json` and reviewed card boundaries from `./data/specimen-card-projections.json`. The schema-15 root contract is `{ metadata, records }`. Every descriptor declares one of the eleven record models below.
...
This model is restricted to the 85 source-order `antarctic-1980` Appendix 1 rows: 54 cite printed page 47 and 31 cite printed page 48. Every row preserves a unique `ALHA77` specimen ID as both `specimenId` and source `name`, a positive source mass, source classification, Allan Hills locality facts, nullable source olivine Fa, pyroxene Fs, and weathering, and an accepted `official-abbreviation` MetBull mapping. The 85 masses total 89,891.1 g and none is null; 9 olivine Fa, 7 pyroxene Fs, and 6 weathering values are null because those source values are unavailable, not because they are zero. Appendix 2 is excluded. The model creates 85 independent observations and does not merge them with the later Victoria Land rows.
...
A `caption-observation-fact` contains exactly:
...
Haag 2003 contributes 250 caption observations across 134 cited pages. Caption mass and dimensions use `caption-context-only` semantics and never become normalized specimen facts. Repeated-view relations point only to an earlier public Haag caption record. Haag contributes no specimen cards, MetBull mappings, projection entries, lineage endpoints, or comparison candidates.
...
Hamburg contributes four reviewed cross-source comparison candidates retained as possible comparisons. They document matching public facts for review and do not establish lineage, specimen identity, custody, ownership, or transfer.
...
Records are source observations, not canonical meteorites. An `appendix-specimen` or `table-a-specimen` observation explicitly describes one source-identified individual specimen; a `regional-census-fact` explicitly does not. Equal names, designations, masses, or page numbers do not merge observations across catalogs. Statistics count each parent record once, count every Antarctic and Victoria Land mass once, include no Hodge-Smith mass, and never multiply a reported mass by holding count.

### Harmonized Public Cards

The public presenter derives 25,244 display cards from 20,241 parent observations and the reviewed projection manifest: 5,824 direct specimens, 8,410 projected atomic specimens, 6,867 collection observations, 3,447 collection-representation observations, 250 caption observations, 84 regional observations, 353 regional-event observations, 6 dealer observations, and 3 source observations explicitly classified as not individual specimens. Projection changes display-card multiplicity only; statistics, citations, folio authorization, and source data remain parent-observation based. The default result count reports filtered specimen cards, while the explicit inclusive view reports its display-card and parent-observation totals.

Every card's top identifier starts with the established concise catalog label. The 13,881 available typed source identifiers are appended unchanged, for example **Nininger (1933) · 9b**, so equal source numbers from different catalogs remain distinguishable; this total includes the 250 Haag captions rendered as **Caption observation &lt;entryOrder&gt;**. Another 5,058 projected cards append an exact evidence-bound **Catalog no.** value; the remaining 6,305 cards show the concise catalog shortname alone. Projected cards divide into 241 typed, 5,058 numbered, and 3,111 shortname-only identifiers; direct specimen cards contain 5,786 typed and 38 shortname-only identifiers. The 3,153 internal entry-order values on other record models are never presented as source identifiers. Stored source identifiers, internal order, searching, and sorting remain unchanged.

Every direct or projected specimen card uses one model-independent primary contract: heading **Name**, followed only by available **Class**, **Place**, **Fall / find**, **Year / date**, **Form**, **Find location**, and **Weight** rows in that order. A mapped specimen uses reviewed current name, class, place, fall/find, and year values; an unavailable current value falls back to the corresponding source value only when that value is actually recorded. Unmapped specimens use source values without inference. **Form** is **Individual specimen** for projected atomic, Appendix, and Table A cards and **Specimen** for generic direct specimens. The sidecar never supplies specimen weight, find location, form, coordinates, or lineage.

All non-contract specimen presentation is inside the initially collapsed **Show specimen notes** disclosure. This includes differing source name, class, place, and date; source description; Victoria conflict rows; Antarctic and Victoria locality, grid, chemistry, weathering, and source-section details; and the existing complete lineage and comparison disclosures. Equivalent source context is not repeated. Source citation and catalog identifier remain structural footer/header metadata. Unmapped specimen cards and sidecar failure still use the same neutral specimen labels with source-only values. Observation headings, fact labels, descriptions, and **Show catalog notes** disclosure remain unchanged.

Complete field-aware unavailability statements are omitted, while compound statements that retain evidence remain verbatim. The neutral specimen labels are identical in visible and accessible text; note-only abbreviations retain complete assistive text. The 4.5-5.75 rem specimen label columns remain side by side through 520 px and stack only below 350 px, without changing font sizes; observation cards retain their established 520 px stack. General cards do not infer catalog-specific holdings, provenance, amendments, totals, or occurrence counts as primary fields. Fletcher context cards label event text **Date or report of find**, show **Reference** separately, and retain section, pane/case, and represented-weight text while stating that the observation is not a specimen, holding, or inventory identity.

The default unchecked filter shows exactly 14,062 source-listed specimen cards: 14,051 with finite numeric weight and 11 with reviewed qualitative weight evidence. It excludes all 11,010 observations and 172 source-unlisted specimen cards. Selecting **Include observations and specimens without weight** explicitly restores the complete 25,244-card register. Brown and Minnesota contribute 538 source-listed cards to the default view; their 11 former catalog-wide fallback cards are no longer treated as weighted without card-specific evidence.

`direct-specimen` and `projected-atomic-specimen` cards are specimen presentations. `source-observation`, `collection-observation`, `collection-representation-observation`, `caption-observation`, `regional-observation`, `regional-event-observation`, and `dealer-observation` cards are explicitly observations and omit specimen-only fields. **Form** is controlled only by reviewed card kind: 8,768 projected atomic, Appendix, or Table A cards display **Individual specimen**, while 5,466 generic direct cards display **Specimen**. It is not inferred from locality, coordinates, names, MetBull data, or descriptive prose. General source locality can populate **Place** but cannot populate **Find location**. The 111 typed specimen locations display exactly; the other 14,123 specimen cards omit unavailable **Find location**.

### Reviewed Specimen Cards

`specimen-card-projections.json` is a schema-6 display-only positive allowlist whose metadata binds the schema-15 catalog, its 20,241 records, and its exact source hash. Every card references an immutable parent observation and exact public holding, then uses one closed evidence variant: a reviewed UTF-16 clause span, a qualitative clause span, or an exact typed Hamburg `componentPath`. Any variant may additionally carry closed `sourceCatalogNumber` evidence containing only the exact public value, same-holding text path, and UTF-16 half-open span. A clause card may carry a closed `repeatedMass` object with `massPath: null`; a qualitative clause card carries an affirmative source statement but no numeric mass. The manifest does not create observation or canonical-specimen IDs or copy unrestricted source prose. Grouped holdings, aggregate components, associated material, representations, counts, ranges, totals, dealer offers, and otherwise unprojected material remain observation or audit context.

The manifest covers 3,407 parent observations and 8,410 atomic specimen cards: 8,375 cards bind an exact `massPath`, 2 Kuleschowka cards bind the same 2.7 g per-item value through distinct `repeatedMass` occurrences, and 35 cards have no normalized display mass, including 10 qualitative cards. It retains 2,877 source-context audit partitions that are not rendered as specimens. Exact reviewed source-number coverage is Farrington 1903 (232), Farrington 1916 (1,100), Reeds 1937 (2,988), Prior 1923 (653), Tassin 1902 (84), and Merrill 1916 (1). Ensisheim remains 207 and 208; Nininger's 241 projected typed designations are not duplicated. Parent-row, ambiguous/multiple, external, private-only, and absent values receive no annotation. The six reviewed catalogs have blocked or undetermined folio policies, so visual source comparison requires their cited editions rather than in-app facsimiles. Bonn contributes 348 reviewed cards across 345 parents, including seven qualitative `Spl.` cards; its collection totals remain context and never become cards. Berlin contributes 838 reviewed cards; Greifswald, Fletcher, Story, Prior representation, regional-event, and Haag caption catalogs contribute none. Search matching and statistics begin from all 20,241 parents across 56 catalogs; the default card filter then retains only source-listed specimens. Numeric comparison routing remains bound to exact non-null `massPath` values.
...
The historical `name`, designation/catalog identifier fields, printed private weight strings, and numeric source weights are never replaced by this object. For a specimen with valid mapped sidecar context, the current Official name becomes the linked **Name** heading regardless of display equivalence; differing source name, classification, locality, and event values move to collapsed specimen notes. Unresolved mappings and sidecar failure retain source-only specimen values under the same neutral labels; observations remain unchanged. Comparison uses Unicode NFC, collapsed whitespace, and locale-aware lowercase. No client, build, or export path fuzzy-matches names or infers identity.

<!-- release-summary:readme-metbull:start -->
The current release includes reviewed MetBull harmonization for 15,299 of 20,241 records: 14,606 have a resolved current identity and 693 remain explicitly unresolved. 82 records currently have a null `name` value.
<!-- release-summary:readme-metbull:end -->

The remaining 4,942 records are pending observations without reviewed MetBull mappings. Story-Maskelyne 1872 contributes 132 pending observations, Prior 1926 contributes 47, and all 353 Bonn observations have closed reviews. Foote's six dealer observations and Haag's 250 caption observations intentionally have no MetBull mapping or current-name claim.
...
The precedence boundary is strict. Historical source name, classification, locality, event wording, specimen mass, coordinates, description, and other specimen facts remain immutable and searchable in `catalog.json`. The reviewed record-level `meteoriteCode` establishes meteorite-event identity. Only then may the sidecar supply current name, class, place, year/date, and fall/find values. Differing historical context remains available under collapsed **Show specimen notes**; equivalent repetition may be omitted, but no source fact is deleted. All observations remain source-only, and unmapped specimen cards use recorded source fallbacks without inference.
...
Fall/find display follows MetBull Database Note 4: `Y` renders **Fall**, an empty value renders **Find**, `Yc` renders **Confirmed fall (Yc)**, `Yp` renders **Probable fall (Yp)**, and `Np` renders **Find, possible fall (Np)**. These are event categories, not mapping confidence. `Nd` occurs in the acquired source but is not used by any current mapped card and is unsupported by the public sidecar; a future mapped occurrence requires explicit reviewed handling rather than fallback inference. Missing or invalid sidecar data falls back to source-only cards. The lineage isolation test proves that the sidecar's presence, absence, or changed current fields cannot alter lineage bytes, relationship or candidate IDs, evidence strength, grouping, or counts; the lineage artifact remains exactly `834b766339614485513d50e5efdcfac0c66b3a9871de73b8533f1978f459fbd2`.
