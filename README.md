...
Every specimen card uses one closed standard vocabulary. After the specimen identifier, semantic type, and source catalog meteorite name, every specimen displays these fact rows in order: **Class**, **Specimen form**, **Source locality**, **Individual find location**, **Event**, **Lineage**, and **Specimen weight**. **Current Meteoritical Bulletin name** is inserted before those rows only when a reviewed current name differs substantively from the source-catalog name. Equivalent and unresolved current names omit the row. Unavailable values in the standard rows read **Unknown**. General cards do not insert catalog-specific holdings, provenance, coordinate, mineral-chemistry, amendment, total, or occurrence-count labels. Fletcher context cards label the heading **Source catalog meteorite or locality name**, label event text **Date or report of find**, show **Reference** separately, and retain section, pane/case, and represented-weight text while stating that the observation is not a specimen, holding, or inventory identity.
