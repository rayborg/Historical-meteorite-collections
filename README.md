...
The repository also publishes [`data/specimen-lineages.json`](./data/specimen-lineages.json), a schema-4 index that keeps attested lineage separate from cross-catalog comparison. Its 194 `same-inventory` relationships record series-scoped inventory-designation continuity, and its 21 source-attested groups preserve n-ary source claims. The 1,541 comparison groups contain 2,245 mass-based candidates for research and review, not lineage assertions. Reviewed Meteoritical Bulletin identity identifies the meteorite event, not a physical specimen; reported mass is comparison evidence only. Candidate decisions are maintained in [`data/specimen-comparison-reviews.json`](./data/specimen-comparison-reviews.json). The separate [`data/specimen-card-projections.json`](./data/specimen-card-projections.json) manifest identifies reviewed source holdings that may be displayed as individual specimen cards without splitting or replacing their parent observations. Schema 6 binds exact same-card public evidence for 5,058 source-reviewed projected catalog numbers across six catalogs while excluding ambiguous, external, parent-only, private-only, and absent values.
...
When historical source name, classification, locality, or event differs from mapped current context, the exact source value remains searchable and appears under the initially collapsed **Show catalog notes** disclosure. Equivalent source context is not repeated there. Unmapped specimen cards and all observation cards remain source-only, as do all cards when the optional sidecar fails validation. Their populated headings continue to use the kind-correct **Meteorite name**, **Source name**, **Meteorite or locality name**, or **Catalog name** label; null names retain the **Source catalog identifier** or **Source catalog record** fallback. Source-only specimen facts retain **Catalog classification**, **Source locality**, and **Event** in their established order.
...
[`data/metbull-current-context.json`](./data/metbull-current-context.json) is a schema-1 display sidecar for current meteorite-event context on specimen cards. It is SHA-256 `9ac2997e1386cc9c2bf3b134f742aa30e5c4bdd23ac9233d18f57900d741d3f3` and is bound to the exact catalog, projection, lineage, and exhaustive card assignment. Its 14,149-card audit maps 11,774 cards from 7,651 parents to 2,548 reviewed Official meteorite codes and leaves 2,375 cards unmapped. Direct cards partition as 5,568 mapped and 171 unmapped; projected cards partition as 6,206 mapped and 2,204 unmapped.

Two different digests protect that boundary. Public runtime binding SHA-256 `2729240dd574b40ed5b9526d0ab81e99108b80894683c9b69157bb9576ef7504` covers the ordered 14,149 public card inputs: card key, direct/projected route, parent ID, projected position, mapped/unmapped status, meteorite code, and canonical name. The runtime recomputes it from public catalog and projection data before accepting the sidecar, so routing or mapping drift fails closed. The sidecar's `cardAssignmentsSha256` value `a732537ef524872ec7bce3340a04db1eb0694453cc158b6497cfd363a973bb44` is the separate private-generation provenance digest; it also binds each assignment to the full Official-row hash. Only that digest value is public. The row hashes and assignment tuples remain in the private audit and are not exposed.

The precedence boundary is strict. Historical source name, classification, locality, event wording, specimen mass, coordinates, description, and other specimen facts remain immutable and searchable in `catalog.json`. The reviewed record-level `meteoriteCode` establishes meteorite-event identity. Only then may the sidecar supply the current Official name, classification, place, year, and fall/find context. Differing historical source context remains available under the collapsed **Show catalog notes** disclosure; equivalent repetition may be omitted from that disclosure, but no source fact is deleted. All observations and all unmapped specimen cards remain source-only.
