...
This repository is a dependency-free, facts-only index of 2,276 source observations from 8 historical meteorite catalogs. The coordinated catalog uses public metadata schema 6 and supports 4 source-specific record models: `catalog-item`, `catalog-number`, `collection-entry`, `specimen`.
...
`nininger-1933` has 136 records; its metadata source pages span 1-20, and its record citations span 1-11. `nininger-1950` has 364 records; its metadata source pages span 26-40, and its record citations span 26-40.
...
The current release includes reviewed MetBull harmonization for 2,276 of 2,276 records: 2,183 have a resolved current identity and 93 remain explicitly unresolved. 7 records currently have a null `name` value.
