...
On a mapped specimen card, the Official **Current MetBull meteorite name** is the heading and links to the canonical detail URL for its reviewed code. When the historical source name differs, it appears first as a visible **Source name** fact so source-name ordering and searches remain self-explanatory. Primary current event context follows as **Current MetBull classification**, **Current MetBull place**, **Current MetBull fall/find**, and **Current MetBull year**. Specimen-specific source facts remain primary: **Specimen form**, **Individual find location**, **Lineage**, cross-catalog comparison status, and **Specimen weight** follow when applicable. The sidecar never supplies specimen mass, coordinates, form, or lineage.

When historical classification, locality, or event differs from mapped current context, the exact source value remains searchable and appears under the initially collapsed **Show catalog notes** disclosure. A differing historical source name remains searchable and is shown directly in the primary fact grid instead. Equivalent source context is not repeated. Unmapped specimen cards and all observation cards remain source-only, as do all cards when the optional sidecar fails validation. Their populated headings continue to use the kind-correct **Meteorite name**, **Source name**, **Meteorite or locality name**, or **Catalog name** label; null names retain the **Source catalog identifier** or **Source catalog record** fallback. Source-only specimen facts retain **Catalog classification**, **Source locality**, and **Event** in their established order.

Complete field-aware unavailability statements are omitted, while compound statements that retain evidence remain verbatim. **Specimen form** appears only as **Individual specimen** when that narrower form is established; the tautological **Specimen form: Specimen** row is omitted. Direct and projected specimen cards compact visible labels only: for example, **Current MetBull meteorite name** displays as **Official name**, **Catalog classification** as **Source class**, and **Specimen weight** as **Weight**. The complete labels remain unchanged in DTOs and in visually hidden assistive text. Their 4.5-5.75 rem label columns remain side by side through 520 px and stack only below 350 px, without changing font sizes; observation cards retain their established 520 px stack. General cards do not insert catalog-specific holdings, provenance, coordinate, mineral-chemistry, amendment, total, or occurrence-count labels. Fletcher context cards label event text **Date or report of find**, show **Reference** separately, and retain section, pane/case, and represented-weight text while stating that the observation is not a specimen, holding, or inventory identity.
...
The historical `name`, designation/catalog identifier fields, printed private weight strings, and numeric source weights are never replaced by this object. For a specimen with valid mapped sidecar context, the current Official name becomes the linked heading regardless of display equivalence; an exact differing source name remains visible as a primary source-scoped fact, while differing classification, locality, and event values move to collapsed catalog notes. Unresolved mappings, observations, and sidecar failure retain source-only presentation. Comparison uses Unicode NFC, collapsed whitespace, and locale-aware lowercase. No client, build, or export path fuzzy-matches names or infers identity.
...
The precedence boundary is strict. Historical source name, classification, locality, event wording, specimen mass, coordinates, description, and other specimen facts remain immutable and searchable in `catalog.json`. The reviewed record-level `meteoriteCode` establishes meteorite-event identity. Only then may the sidecar supply the current Official name, classification, place, year, and fall/find context. A differing historical source name is visible in the primary fact grid; other differing source context remains available under the collapsed **Show catalog notes** disclosure. Equivalent repetition may be omitted, but no source fact is deleted. All observations and all unmapped specimen cards remain source-only.
