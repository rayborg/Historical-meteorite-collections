...
This repository is a dependency-free, facts-only index of 18,217 source observations from 49 historical meteorite catalogs. The coordinated catalog uses public metadata schema 11 and supports 8 source-specific record models: `catalog-item`, `catalog-number`, `collection-entry`, `collection-representation-fact`, `dealer-offer-fact`, `regional-census-fact`, `specimen`, `table-a-specimen`. Reviewed projection metadata expands 3,062 parent observations into 8,062 atomic specimen cards, producing 23,217 searchable display descriptors: 13,804 specimen cards and 9,413 observations.
...
The validator checks both synthetic rejection fixtures and the real catalog, manifest, and folio files, including exactly 22 catalog-number rejection cases. The standalone runtime harness contains 124 tests, and the complete MJS suite contains 178 tests.
...
The browser loads factual records from `./data/catalog.json` and reviewed card boundaries from `./data/specimen-card-projections.json`. The schema-11 root contract is `{ metadata, records }`. Every descriptor declares one of the eight record models below.
...
The public presenter derives 23,217 display cards from 18,217 parent observations and the reviewed projection manifest: 5,742 direct specimens, 8,062 projected atomic specimens, 6,859 collection observations, 2,464 collection-representation observations, 84 regional observations, and 6 dealer observations. Projection changes display-card multiplicity only; statistics, citations, folio authorization, and source data remain parent-observation based. The default result count reports filtered specimen cards, while the explicit inclusive view reports its display-card and parent-observation totals.
...
The default unchecked filter shows 13,631 `direct-specimen` or `projected-atomic-specimen` cards with known weight under the reviewed source-bound rule. It excludes all 9,413 collection, collection-representation, regional, and dealer observations, including observations that carry contextual masses, and excludes 173 unknown-mass specimen cards. Selecting **Include observations and specimens without weight** explicitly restores the complete 23,217-card register. Brown's 385 and Minnesota's 164 reviewed specimen cards remain in the default results under the accepted harmonized rule even though 11 lack a specific mass path.

`direct-specimen` and `projected-atomic-specimen` cards are specimen presentations. `collection-observation`, `collection-representation-observation`, `regional-observation`, and `dealer-observation` cards are explicitly observations and omit specimen-only fields. **Specimen form** is controlled only by reviewed card kind; it is not inferred from locality, coordinates, names, MetBull data, or descriptive prose. **Source locality** preserves the catalog's locality scope and is not an individual find-location field. The 111 typed specimen locations display exactly; the other 13,693 specimen cards display **Individual find location: Unknown**.

### Reviewed Specimen Cards

`specimen-card-projections.json` is a schema-4 display-only positive allowlist whose metadata binds the schema-11 catalog, its 18,217 records, and its exact source hash. Every card references an immutable parent observation and exact public holding, then uses one evidence variant: a reviewed UTF-16 clause span in an already-public description or designation, or an exact typed Hamburg `componentPath`. Schema 4 also permits a closed `repeatedMass` object on a clause card with `massPath: null`; it binds one per-item value, count, aggregate total, occurrence, and occurrence count in the same holding. It does not create observation or canonical-specimen IDs or copy source prose into the manifest. Grouped holdings, aggregate components, associated material, representations, counts, ranges, totals, dealer offers, and otherwise unprojected material remain observation or audit context and are not promoted into specimen claims.

The manifest covers 3,062 parent observations and 8,062 atomic specimen cards: 8,032 ordinary cards bind an exact `massPath`, 2 Kuleschowka cards bind the same 2.7 g per-item value through distinct `repeatedMass` occurrences, and 28 cards have no normalized display mass. It retains 2,532 source-context audit partitions that are not rendered as specimens. Berlin contributes 838 reviewed cards; Greifswald and Fletcher contribute none because their source structures do not establish atomic specimen display units. Brown contributes 385 reviewed cards and Minnesota 164; group context and excluded non-meteorite context remain unrendered. Hamburg contributes 218 component cards across 142 parents, Madrid contributes 54 cards across 23 parents, Reeds entry 366 produces ten cards, and Prior entry 630 produces seventeen. The allowlist does not project dealer, regional-census, collection-representation, or Table A observations. Search matching and statistics begin from all 18,217 parents across 49 catalogs; the default card filter then retains only weighted specimens. Fletcher represented weights remain source context and are excluded from specimen mass filtering and lineage. Lineage is routed only by exact non-null `massPath`; reviewed relationships remain possible matches rather than identity, custody, ownership-transfer, or merge claims.
