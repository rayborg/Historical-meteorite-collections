...
This repository is a dependency-free, facts-only index of 13,365 source observations from 30 historical meteorite catalogs. The coordinated catalog uses public metadata schema 6 and supports 4 source-specific record models: `catalog-item`, `catalog-number`, `collection-entry`, `specimen`.
...
The current release includes reviewed MetBull harmonization for 10,025 of 13,365 records: 9,896 have a resolved current identity and 129 remain explicitly unresolved. 13 records currently have a null `name` value.
