...
This repository is a dependency-free, facts-only index of 3,565 source observations from 8 historical meteorite catalogs. The coordinated catalog uses public metadata schema 6 and supports 4 source-specific record models: `catalog-item`, `catalog-number`, `collection-entry`, `specimen`.
...
The current release includes reviewed MetBull harmonization for 3,565 of 3,565 records: 3,443 have a resolved current identity and 122 remain explicitly unresolved. 7 records currently have a null `name` value.
