...
The public presenter derives 18,896 display cards from the unchanged 14,176 parent observations and reviewed projection manifest: 5,742 direct specimens, 6,675 projected atomic specimens, 6,395 collection observations, and 84 regional observations. Projection changes display-card multiplicity only; search result counts, statistics, citations, folio authorization, and source data remain parent-observation based.

Every card uses one closed standard vocabulary. After the source identifier, semantic type, and source catalog meteorite name, fact rows appear in this order when applicable: differing reviewed **Current Meteoritical Bulletin name**, **Class**, **Specimen form**, **Source locality**, **Event**, **Lineage**, and **Specimen weight**. Regional observations then add the three controlled Australian Museum representation facts. Missing standard values read **Not recorded**. Display-equivalent current names are suppressed, unresolved reviews do not create a current-name row, and no catalog-specific holdings, provenance, coordinate, mineral-chemistry, amendment, total, or section labels are inserted into cards. Those public source facts remain searchable.

`direct-specimen` and `projected-atomic-specimen` cards are specimen presentations. `collection-observation` and `regional-observation` cards are explicitly observations and omit specimen form, lineage, and specimen weight. **Specimen form** is controlled only by reviewed card kind; it is not inferred from locality, coordinates, names, MetBull data, or descriptive prose. **Source locality** preserves the catalog's locality scope and is not an individual find-location field. This release does not claim that any card provides an exact individual find location.
...
The historical `name`, designation/catalog identifier fields, printed private weight strings, and numeric source weights are never replaced by this object. Resolved reviews produce a **Current Meteoritical Bulletin name** row only when it differs substantively from the displayed source name; comparison uses Unicode NFC, collapsed whitespace, and locale-aware lowercase solely to suppress redundant values. Unresolved reviews do not create a current-name row. No client, build, or export path fuzzy-matches names or infers identity.
