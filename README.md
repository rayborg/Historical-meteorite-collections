...
This repository is a dependency-free, facts-only index of 1,984 source observations from eight historical meteorite catalogs. The coordinated catalog uses public metadata schema 6 and supports four source-specific record models: `specimen`, `catalog-item`, `catalog-number`, and `collection-entry`.
...
| `nininger-1950` | `specimen` | 72 | 6 | 6 |
| **Total** |  | **1,984** | **154** | **137** |

Metadata source-page coverage is not a claim that every covered page contains a record. Chladni 1825 pages 200-207 are introductory folios; records begin on page 208. The Nininger 1933 source set has a title page and printed pages 1-7 and 10-20. Printed pages 8-9 and catalog items 106-141 are missing. Pages 12-20 are narrative-only and contain no catalog-item observations. The resulting 136 entries form a partial, not complete, digital edition. Nininger 1950 currently covers printed pages 26-31 and 72 specimen observations from Abancay through Bear Creek; page 30 ends with the complete Bald Mountain entry before page 31 begins Ballinger. Beardsley begins at the foot of page 31 but contributes no observation there because its specimen section starts after the page boundary.
...
The current release reviews all 1,984 records: 1,891 have a resolved current identity and 93 remain explicitly unresolved. Validated continuation evidence recovers source names for 218 formerly blank rows. Seven historical entries genuinely print no separate proper source name; they retain null names and unresolved reviews without an inferred modern identity.
