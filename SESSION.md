...
- Current Official name, classification, place, year, and fall/find are meteorite-event context only. Historical source and specimen facts remain immutable and searchable; a differing source name is directly visible and other differing source context remains under collapsed `Show catalog notes`, while observations and unmapped cards remain source-only. Current class is not match likelihood because reviewed code identity is already established. Official mass, coordinates, comments, and the raw CSV are excluded.
