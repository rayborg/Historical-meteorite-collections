...
- Cards display only known, nonredundant facts and source identifiers; unavailable fields and the tautological `Specimen form: Specimen` row are omitted. Every displayed source identifier is prefixed with its established concise catalog label to disambiguate equal numbers across catalogs without mutating source values. Specimen headings use `Meteorite name`, while the narrower established form remains `Individual specimen`. A specimen card includes the current-name field only when a reviewed canonical name differs substantively from the source-catalog name. Lineage disclosures name the earlier catalog and retain stable IDs inside expanded audit details. Card facts use compact aligned columns at usable widths and stack below 520 px without smaller typography. The homepage links to the dedicated catalog directory and separately to the bibliography master list.
