...
- A specimen card includes the current-name field only when a reviewed canonical name differs substantively from the source-catalog name; equivalent and unresolved names omit the row. Card facts use compact aligned columns at usable widths and stack below 520 px without smaller typography or omitted fields. The homepage links to the dedicated catalog directory and separately to the bibliography master list.
