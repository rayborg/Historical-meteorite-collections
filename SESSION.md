...
- The schema-6 projection manifest contains 3,407 projected parents, 8,410 atomic cards, 2,877 non-displayed context audit partitions, and 232 exact source catalog numbers on all and only the reviewed projected Farrington 1903 cards. Ensisheim resolves to printed numbers 207 and 208. The 192 nonprojected Farrington holdings and four multi/range holdings remain unannotated; no other catalog number coverage is claimed. Existing paths, clauses, masses, card order, parent IDs, and lineage data remain unchanged.
- Every one of the 24,556 card identifiers starts with its concise catalog shortname. The 13,440 typed source identifiers retain their wording, 232 Farrington cards append `Catalog no.`, and 10,884 cards use the shortname alone; internal entry order is never used as a fallback.
- The harmonized presenter derives 24,556 cards from 19,553 parent observations: 5,739 direct specimens, 8,410 projected atomic specimens, 6,867 collection observations, 3,447 collection-representation observations, 84 regional observations, 6 dealer observations, and 3 not-individual source observations. The 14,149 specimen cards use the standard eight-field order. Story and Prior context cards preserve representation facts without specimen claims.
...
- The regenerated lineage index contains 2,439 relationships: 194 identity-consistent same-inventory relationships and 2,245 possible cross-source matches from 17,160 positive flattened masses. Incompatible Nininger inventory `139b` is omitted and counted once as ambiguous; no unrelated relationship changed. Of the possible candidates, 1,339 are unreviewed and 906 are reviewed, including 10 Bonn candidates retained as possible only. None asserts physical identity, custody, ownership, transfer, or merge.
- Source-complete card routing exposes 963 cards and 1,451 closed provenance claims in the inclusive lineage-only view: 194 known same-inventory claims, 1,178 suspected cross-catalog claims, and 79 tentative source-attested group claims. Claims retain applicable endpoint, citation, evidence, review, and caution details rather than only a count summary.
...
- `story-maskelyne-1872` and `prior-guide-1926` add 303 and 680 source-local collection-representation observations with zero specimen cards or lineage endpoints. `brauns-bonn-1926` adds 353 collection entries, 348 reviewed atomic cards, and 10 reviewed possible lineages without same-inventory or merge claims. All three folios and source media remain blocked.
...
- Cards display only known, nonredundant facts and source identifiers; complete field-aware unavailability statements and the tautological `Specimen form: Specimen` row are omitted, while compound evidence remains. Every card starts with a concise catalog shortname: 13,440 typed source identifiers retain their suffixes, 232 reviewed projected Farrington cards append an evidence-bound catalog number, and 10,884 cards use the shortname alone. The 2,906 internal entry-order values never become display identifiers. Populated headings use kind-correct `Meteorite name`, `Source name`, `Meteorite or locality name`, or `Catalog name` labels, while the narrower established specimen form remains `Individual specimen`. A specimen card includes the current-name field only when a reviewed canonical name differs substantively from the source-catalog name. Lineage disclosures name the earlier catalog without rendering stable relationship or group IDs or unavailable detail rows; those IDs remain in data, DTOs, and routing. Card facts use compact aligned columns at usable widths and stack below 520 px without smaller typography. The homepage links to the dedicated catalog directory and separately to the bibliography master list.
