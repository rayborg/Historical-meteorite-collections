...
- Schema 12 contains 19,553 facts-only records across 52 catalogs.
- Reviewed display projections replace 3,407 parents with 8,410 atomic cards: 24,556 descriptors comprise 14,149 specimens and 10,407 observations; weighted-only display excludes 172 unknown-mass specimens.
...
- Metadata covers 1,733 catalog-scoped source pages; records cite 1,556 of them.
...
- Blocked folio catalogs are: `anderson-1913` (0 pages, `undetermined`); `astapovich-1938` (0 pages, `undetermined`); `asu-2024-09` (0 pages, `undetermined`); `ball-1882` (0 pages, `undetermined`); `barnes-1940` (0 pages, `undetermined`); `berlin-1903` (0 pages, `undetermined`); `berlin-1904` (0 pages, `undetermined`); `brauns-bonn-1926` (0 pages, `undetermined`); `brown-1916` (0 pages, `undetermined`); `buchner-1863` (0 pages, `undetermined`); `chladni-1825` (0 pages, `undetermined`); `farrington-1903` (0 pages, `undetermined`); `farrington-1916` (0 pages, `undetermined`); `fletcher-1886` (0 pages, `undetermined`); `fletcher-1894` (0 pages, `undetermined`); `fletcher-1896` (0 pages, `undetermined`); `fletcher-1904` (0 pages, `undetermined`); `fletcher-1908` (0 pages, `undetermined`); `foote-1909` (0 pages, `undetermined`); `foote-1912` (0 pages, `undetermined`); `greifswald-1895` (0 pages, `undetermined`); `greifswald-1901` (0 pages, `undetermined`); `hamburg-1913` (0 pages, `undetermined`); `hodge-smith-1939` (0 pages, `undetermined`); `hogbom-1902` (0 pages, `undetermined`); `huss-1976` (0 pages, `undetermined`); `huss-1986` (0 pages, `undetermined`); `kanagawa-1996` (0 pages, `undetermined`); `kantor-1920` (0 pages, `undetermined`); `madrid-1923` (0 pages, `undetermined`); `mason-1964` (0 pages, `undetermined`); `merrill-1916` (0 pages, `undetermined`); `minnesota-1892` (0 pages, `undetermined`); `nininger-1950` (0 pages, `undetermined`); `nordenskiold-1870` (0 pages, `undetermined`); `palache-1926` (0 pages, `undetermined`); `prior-1923` (0 pages, `undetermined`); `prior-guide-1926` (0 pages, `undetermined`); `reeds-1937` (0 pages, `undetermined`); `schreiter-1912` (0 pages, `undetermined`); `story-maskelyne-1872` (0 pages, `undetermined`); `tassin-1902` (0 pages, `undetermined`); `usnm-1886` (0 pages, `undetermined`); `victoria-land-1982` (0 pages, `undetermined`); `ward-1881` (0 pages, `undetermined`); `ward-1904` (0 pages, `undetermined`); `washington-1897` (0 pages, `undetermined`).
- Reviewed MetBull harmonization covers 14,861 of 19,553 records: 14,521 resolved and 340 explicitly unresolved.
- Records currently having a null `name` value: 70.
<!-- release-summary:session-current-state:end -->

- The remaining 4,692 records are pending observations without reviewed MetBull mappings.
- The schema-1 current MetBull specimen-context sidecar is SHA-256 `9ac2997e1386cc9c2bf3b134f742aa30e5c4bdd23ac9233d18f57900d741d3f3`. Its exhaustive 14,149-card assignment maps 11,774 cards from 7,651 parents to 2,548 Official codes and leaves 2,375 cards unmapped. The direct partition is 5,568 mapped/171 unmapped; the projected partition is 6,206 mapped/2,204 unmapped.
- Public card-binding SHA-256 `2729240dd574b40ed5b9526d0ab81e99108b80894683c9b69157bb9576ef7504` covers all 14,149 ordered card keys, routes, parents, projected positions, mapping statuses, codes, and canonical names and fails closed on public mapping drift. Distinct sidecar provenance digest `a732537ef524872ec7bce3340a04db1eb0694453cc158b6497cfd363a973bb44` additionally includes full Official-row hashes in the private audit; only the digest value is public.
- Current Official name, classification, place, year, and fall/find are meteorite-event context only. Historical source and specimen facts remain immutable and searchable; differing source context remains under collapsed `Show catalog notes`, while observations and unmapped cards remain source-only. Current class is not match likelihood because reviewed code identity is already established. Official mass, coordinates, comments, and the raw CSV are excluded.
- Current fall code `Y` displays as `Fall` and empty as `Find`; exceptional `Yc`, `Yp`, and `Np` display literally as `Code <value>` rather than being interpreted as confidence. `Nd` exists in the 80,224-row acquired CSV but is unused, unsupported by the public sidecar, and requires explicit handling before any future use.
- Private provenance remains at `data/private/metbull-current-context-2026-09-12/acquisition.json` and `data/private/metbull-current-context-2026-09-12/specimen-card-audit.json`. The CSV SHA-256 is `1e22fb5cac0e46628e73e74f2ad3dac15240fd247ccffa621bf89539c5b18d68`; the audit SHA-256 is `8bb453b7a970e14bf877fdd32c81e9b7b1888a97433a13e0410c099e42f514b9`.
...
- Every one of the 24,556 card identifiers starts with its concise catalog shortname. The 13,440 typed source identifiers retain their wording, 5,058 projected cards append `Catalog no.`, and 6,058 cards use the shortname alone. Projected cards divide into 241 typed, 5,058 numbered, and 3,111 shortname-only identifiers; direct cards remain 5,701 typed and 38 shortname-only. Internal entry order is never used as a fallback.
- The harmonized presenter derives 24,556 cards from 19,553 parent observations: 5,739 direct specimens, 8,410 projected atomic specimens, 6,867 collection observations, 3,447 collection-representation observations, 84 regional observations, 6 dealer observations, and 3 not-individual source observations. The 14,149 specimen cards preserve source-specific specimen fields while mapped cards add the linked Official heading and ordered current event context described below. Story and Prior context cards preserve representation facts without specimen claims.
- The unchecked default is strict: it shows 13,977 source-listed specimen cards, partitioned as 13,966 numeric and 11 qualitative; it excludes all 10,407 observations and 172 source-unlisted specimen cards. The inclusive 24,556-card state is explicit and uses `weighted=0`; omitted, malformed, duplicate, and legacy `weighted=1` states remain strict.
- The schema-4 lineage artifact has SHA-256 `b592065b07412b8d09d955f9276b2de0790a56f1649c7987a8b10bcc62c32199`. It contains 194 identity-consistent same-inventory relationships and 21 source-attested groups with 89 occurrences covering 87 unique exact source IDs. True lineage routing exposes 271 cards and 273 claims: 194 known inventory-designation continuity claims and 79 tentative source-attested group occurrences.
...
- Cards display only known, nonredundant facts and source identifiers; complete field-aware unavailability statements and the tautological `Specimen form: Specimen` row are omitted, while compound evidence remains. Every card starts with a concise catalog shortname: 13,440 typed source identifiers retain their suffixes, 5,058 reviewed projected cards append an evidence-bound catalog number, and 6,058 cards use the shortname alone. The 2,906 internal entry-order values never become display identifiers. With valid sidecar context, every mapped specimen uses the linked `Current MetBull meteorite name` heading and primary current classification, place, fall/find, and year; differing exact source name, classification, locality, and event remain searchable under collapsed `Show catalog notes`. Specimen form, individual find location, lineage/comparison status, and specimen weight remain source-specific primary facts. Unmapped specimens, observations, and sidecar failure use kind-correct source-only headings and facts. Lineage disclosures name the earlier catalog without rendering stable relationship or group IDs or unavailable detail rows; those IDs remain in data, DTOs, and routing. Card facts use compact aligned columns at usable widths and stack below 520 px without smaller typography. The homepage links to the dedicated catalog directory and separately to the bibliography master list.
...
1. Preserve the current 52-catalog facts-only public release candidate. Commit or push only when explicitly requested.
...
3. Select the next processable bibliography control only after preserving and validating the current 52-catalog release candidate.
