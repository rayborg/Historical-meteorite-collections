...
test("publishes a safe and accessible bibliography table", async () => {
  ...
  assert.equal((bibliography.match(/<li class="project complete/g) || []).length, 52);
  assert.equal((bibliography.match(/<tr class="worked-row">/g) || []).length, 51);
  assert.equal((bibliography.match(/class="processing-cell complete"/g) || []).length, 51);
  assert.match(bibliography, /<strong>264<\/strong><span>Total controls<\/span>/);
  assert.match(bibliography, /<strong>52<\/strong><span>Catalog projects done<\/span>/);
  assert.match(bibliography, /<strong>51<\/strong><span>Integrated controls<\/span>/);
  assert.match(bibliography, /<strong>23<\/strong><span>Remaining acquired backlog<\/span>/);
  assert.match(bibliography, /Acquisition-to-integration backlog: 23 remaining/);
  ...
  for (const controlId of ["MCB-41", "MCB-62", "MCB-71", "MCB-75", "MCB-80", "MCB-86", "MCB-93", "MCB-94", "MCB-99", "MCB-111", "MCB-117", "MCB-140", "MCB-141", "MCB-142", "MCB-165", "MCB-175", "MCB-197", "MCB-204"]) {
