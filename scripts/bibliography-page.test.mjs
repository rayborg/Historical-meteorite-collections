...
test("publishes a safe and accessible bibliography table", async () => {
  ...
  assert.equal((bibliography.match(/<li class="project complete/g) || []).length, 53);
  assert.equal((bibliography.match(/<tr class="worked-row">/g) || []).length, 52);
  assert.equal((bibliography.match(/class="processing-cell complete"/g) || []).length, 52);
  assert.match(bibliography, /<strong>264<\/strong><span>Total controls<\/span>/);
  assert.match(bibliography, /<strong>53<\/strong><span>Catalog projects done<\/span>/);
  assert.match(bibliography, /<strong>52<\/strong><span>Integrated controls<\/span>/);
  assert.match(bibliography, /<strong>22<\/strong><span>Remaining acquired backlog<\/span>/);
  assert.match(bibliography, /Acquisition-to-integration backlog: 22 remaining/);
