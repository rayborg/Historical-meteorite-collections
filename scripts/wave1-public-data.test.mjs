...
test("retains the accepted Wave 1 data in the schema-14 export", () => {
  ...
  const nonCorrectionRecords = catalog.records.filter(({ id, catalogId }) => catalogId !== "antarctic-1980" && !correctedRecordIds.has(id) && !wave2CatalogIds.has(catalogId) && !fletcherCatalogIds.has(catalogId) && !museum3CatalogIds.has(catalogId) && !catalog10Ids.has(catalogId));
  const nonVictoriaDescriptors = catalog.metadata.catalogs.filter(({ id }) => !["antarctic-1980", "victoria-land-1982"].includes(id) && !wave2CatalogIds.has(id) && !fletcherCatalogIds.has(id) && !museum3CatalogIds.has(id) && !catalog10Ids.has(id));
...
test("Wave 1 public outputs contain no source artifacts or private leakage", () => {
  ...
  assert.equal(Object.keys(folios.catalogs).length, 55);
