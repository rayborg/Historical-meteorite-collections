...
test("production cards suppress equal canonical names, including Aba Panu, while retaining other reviews", () => {
  ...
  assert.deepEqual(
    ...
    { reviewed: 15299, resolved: 14606, unresolved: 693, pending: 4692 }
  );
...
test("production catalog directory preserves all canonical cards and authorized actions", async () => {
  ...
  assert.equal(directory.entries.length, 55);
...
test("homepage and catalog page markup keep navigation and UI states accessible", async () => {
  ...
  assert.match(html, /catalogs\.js\?v=20260919-specimen-card-labels-1/);
  assert.match(html, /app\.js\?v=20260919-specimen-card-labels-1/);
  assert.match(html, /styles\.css\?v=20260919-specimen-card-labels-1/);
