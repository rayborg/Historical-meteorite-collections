...
test("production cards suppress equal canonical names, including Aba Panu, while retaining other reviews", () => {
  ...
  assert.deepEqual(
    ...
    { reviewed: 11177, resolved: 10873, unresolved: 304, pending: 3300 }
  );
  assert.deepEqual({ equivalent: equivalent.length, substantive: resolved.length - equivalent.length }, { equivalent: 9338, substantive: 1535 });
...
test("production catalog directory preserves all canonical cards and authorized actions", async () => {
  ...
  assert.equal(directory.entries.length, 40);
...
test("homepage and catalog page markup keep navigation and UI states accessible", async () => {
  ...
  assert.match(html, /catalogs\.js\?v=20260905-hide-specimen-label-1/);
  assert.match(html, /app\.js\?v=20260905-hide-specimen-label-1/);
  assert.match(html, /styles\.css\?v=20260905-hide-specimen-label-1/);
