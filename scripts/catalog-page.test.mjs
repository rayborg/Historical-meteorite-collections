...
test("production cards suppress equal canonical names, including Aba Panu, while retaining other reviews", () => {
  ...
  assert.deepEqual(
    ...
    { reviewed: 12072, resolved: 11768, unresolved: 304, pending: 3681 }
  );
  assert.deepEqual({ equivalent: equivalent.length, substantive: resolved.length - equivalent.length }, { equivalent: 9338, substantive: 2430 });
...
test("production catalog directory preserves all canonical cards and authorized actions", async () => {
  ...
  assert.equal(directory.entries.length, 44);
...
test("homepage and catalog page markup keep navigation and UI states accessible", async () => {
  ...
  assert.match(html, /catalogs\.js\?v=20260905-catalog-wave2-1/);
