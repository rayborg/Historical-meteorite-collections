...
function fetchFixture(catalogPayload = catalog, folioPayload = folios) {
  return async (url) => ({
    ok: true,
    json: async () => url.includes("catalog.json") ? catalogPayload : folioPayload
  });
}
...
test("production cards suppress equal canonical names, including Aba Panu, while retaining other reviews", () => {
  ...
  assert.deepEqual(
    ...
    { reviewed: 15299, resolved: 14606, unresolved: 693, pending: 4942 }
  );
  assert.deepEqual({ equivalent: equivalent.length, substantive: resolved.length - equivalent.length }, { equivalent: 9620, substantive: 4986 });
...
test("production catalog directory preserves all canonical cards and authorized actions", async () => {
  ...
  assert.equal(directory.entries.length, 56);
...
test("homepage and catalog page markup keep navigation and UI states accessible", async () => {
  ...
  assert.match(html, /catalogs\.js\?v=20260921-specimen-field-contract-1/);
  assert.match(html, /app\.js\?v=20260921-specimen-field-contract-1/);
  assert.match(html, /styles\.css\?v=20260921-specimen-field-contract-1/);
