...
test("production cards suppress equal canonical names, including Aba Panu, while retaining other reviews", () => {
  ...
  assert.deepEqual(
    ...
    { reviewed: 14861, resolved: 14521, unresolved: 340, pending: 4692 }
  );
  assert.deepEqual({ equivalent: equivalent.length, substantive: resolved.length - equivalent.length }, { equivalent: 9620, substantive: 4901 });
...
test("production catalog directory preserves all canonical cards and authorized actions", async () => {
  ...
  assert.equal(directory.entries.length, 52);
...
test("homepage and catalog page markup keep navigation and UI states accessible", async () => {
  ...
  assert.match(html, /catalogs\.js\?v=20260912-catalog-classification-1/);
  assert.match(html, /app\.js\?v=20260912-catalog-classification-1/);
  assert.match(html, /styles\.css\?v=20260912-catalog-classification-1/);
