...
test("main template presents relationship-specific compact lineage copy", async () => {
  ...
  assert.match(html, /styles\.css\?v=20260804-1/);
  assert.match(html, /app\.js\?v=20260804-1/);
...
test("review outcomes apply only to possible matches and not-supported entries are omitted", () => {
  ...
  assert.equal(entryCount(app.deriveEarlierRecordIndex(notSupported, records, registry)), 1465);
...
test("real data maps only later records without mutation and matches the locked distribution", () => {
  ...
  assert.equal(index.size, 932);
  assert.equal(entryCount(index), 1466);
  ...
  assert.deepEqual(distribution, { 1: 758, 2: 130, 3: 16, 4: 8, 5: 4, 6: 4, 7: 1, 8: 2, 9: 2, 10: 1, 15: 1, 18: 1, 25: 1, 36: 1, 81: 1, 98: 1 });
...
test("all 1466 earlier links resolve to exact public source records", () => {
  const index = app.deriveEarlierRecordIndex(lineageData, records, registry);
...
test("all 2932 published observation links resolve to exact public source records", () => {
  let count = 0;
