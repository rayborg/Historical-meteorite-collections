...
test("production manifest is deterministic and validates against the locked catalog", () => {
  ...
  assert.equal(createHash("sha256").update(projectionText).digest("hex"), "de553de6c7be4fa1d57e5efc50cc843a6f1e03e364e8ea8115d71506aa6ed16e");
});
...
test("schema is a closed schema-6 count-locked atomic projection contract", () => {
  ...
  assert.equal(schema.$defs.metadata.properties.catalogSchemaVersion.const, 15);
  assert.equal(schema.$defs.metadata.properties.sourceRecordCount.const, 20241);
  assert.equal(schema.$defs.metadata.properties.sourceCatalogSha256.const,
    "3c23d1c2653bc893819a605c7c0e5e6db7b63a17cd3ab66ab104c2772391dbe7");
...
test("unprojected sources remain unprojected and schema 15 retains specimen locations", () => {
  ...
  assert.equal(published.metadata.catalogSchemaVersion, 15);
  assert.equal(published.metadata.sourceRecordCount, 20241);
});
